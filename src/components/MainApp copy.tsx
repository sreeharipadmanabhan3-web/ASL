...
export function MainApp({ user, onLogout, onNavigateHome, onOpenSettings }: MainAppProps) {
  ...
  return (
    <div className="w-screen h-dvh min-h-dvh animate-bg-fluid flex p-4 gap-4 overflow-hidden select-none animate-fade-in relative">
      ...
      <div className="absolute top-0 right-1/3 w-px h-full bg-gradient-to-b from-transparent via-teal-500/5 to-transparent pointer-events-none z-0" />
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0 h-full overflow-hidden z-10 relative">
        ...
        <div className="flex-1 flex flex-col h-full min-h-0 overflow-hidden relative z-10">
          ...
          <div
            className={`flex-1 h-full min-h-0 flex flex-col overflow-hidden relative glass-panel rounded-3xl transition-all duration-500 ${handsDetected > 0
              ? 'shadow-[inset_0_0_60px_rgba(0,120,212,0.08)]'
              : ''
              }`}
          >
            {/* Video Viewport Area */}
            <div className="flex-1 w-full h-full min-h-0 bg-[#090d16]/30 relative flex items-center justify-center overflow-hidden">
              ...
              {!mlLoaded && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/70 backdrop-blur-md z-30 animate-fade-in pointer-events-auto">
                  <div className="text-center p-6 w-full max-w-sm glass-panel rounded-3xl shadow-2xl flex flex-col items-center">
                    {mlLoading ? (
                      <>
              ...
              {/* Camera Offline / Disconnected / Starting states */}
              {mlLoaded && !isTracking && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-20 animate-fade-in pointer-events-auto">
                  <div className="text-center p-6 w-full max-w-sm glass-panel rounded-3xl shadow-2xl flex flex-col items-center">
                    {status === 'idle' ? (
                      <>
                        {/* Standby State */}
                        ...
                        <button
                          ...
                          className="w-full py-3 rounded-xl bg-gradient-to-r from-[#0078d4] to-[#7c3aed] hover:from-[#0086f0] hover:to-[#8b5cf6] text-white text-xs font-bold tracking-wider uppercase shadow-lg shadow-blue-500/10 hover:shadow-blue-500/20 transition-all duration-300 active:scale-95 cursor-pointer"
                        >
                          Connect Camera
                        </button>
                      </>
                      ...
                      <>
                        ...
                        <div className="relative w-16 h-16 mb-4 flex items-center justify-center">
                          <div className="absolute inset-0 rounded-full border-2 border-white/[0.06]" />
                          <div className="absolute inset-0 rounded-full border-t-2 border-r-2 border-transparent border-t-blue-500 border-r-blue-500/40 animate-spin" />
                          <div className="absolute inset-1.5 rounded-full border-b-2 border-l-2 border-transparent border-b-purple-400 border-l-purple-400/40 animate-spin" style={{ animationDuration: '1.6s', animationDirection: 'reverse' }} />
            ...
            <div className="w-full bg-black/40 border-t border-white/[0.06] flex flex-col gap-3.5 p-4 shrink-0 mb-0 z-20 backdrop-blur-md relative">
                ...
                <div className="flex items-center gap-2.5 flex-wrap">
                  ...
                  {isTracking && (
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-black/40 border border-white/[0.06] shadow-sm animate-fade-in">
                      <div className={`w-1.5 h-1.5 rounded-full transition-colors duration-300 ${handsDetected > 0 ? 'bg-blue-400 shadow-[0_0_6px_rgba(0,120,212,0.8)]' : 'bg-white/25'}`} />
                  ...
                  {isTracking && predictions.length > 0 && (
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-blue-500/10 border border-blue-500/20 shadow-sm animate-fade-in">
                  ...
                  <div className="relative">
                    ...
                    {showDeviceDropdown && (
                      <div className="absolute bottom-full left-0 mb-2 w-64 rounded-xl bg-black/80 backdrop-blur-md border border-white/10 shadow-2xl p-1.5 flex flex-col gap-1 z-40 animate-scale-in">
                        {devices.length === 0 ? (
                        ...
                        {(['480p', '720p', '1080p'] as const).map((res) => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 rounded-lg text-xs transition-all duration-150 flex items-center justify-between cursor-pointer ${res === resolution
                              ? 'bg-blue-500/20 text-blue-400 font-semibold border border-blue-500/10'
                              : 'text-white/70 hover:text-white hover:bg-white/[0.04]'
                              }`}
                          >
                ...
                <div className="flex items-center gap-1.5">
                      ...
                      <div className="relative">
                        ...
                        {showExposureSlider && (
                          <div
                            ...
                            className="absolute right-0 bottom-full mb-2 bg-black/80 backdrop-blur-md p-3.5 rounded-2xl border border-white/10 shadow-2xl flex flex-col gap-1.5 z-40 animate-scale-in w-48 pointer-events-auto"
                          >
                            <div className="flex items-center justify-between text-[9px] font-mono font-bold tracking-wider text-white/50">
                              <span>EXPOSURE</span>
                              <span className="text-blue-400">{Math.round(exposure * 100)}%</span>
                            </div>
                            <input
                              type="range"
                              min="0.5"
                              ...
                              className="w-full accent-blue-500 h-1.5 rounded-lg bg-white/[0.08] cursor-pointer appearance-none outline-none focus:outline-none"
                            />
                          </div>
                        )}
                      </div>
                  ...
                  <button
                    ...
                    className={`p-2 rounded-xl transition-all duration-200 active:scale-90 cursor-pointer border flex items-center justify-center gap-1.5 text-xs font-semibold px-3 ${isTracking
                      ...
                      : 'bg-blue-500/15 hover:bg-blue-500/25 text-blue-400 border-blue-500/30 shadow-lg shadow-blue-500/5'
                      }`}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <div className="flex justify-center">
                  <RecordingControls
                    ...
                    hasPrediction={predictions.length > 0 && predictions[0].word !== 'Gesture Not Found'}
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${mlLoaded ? 'bg-blue-400 shadow-[0_0_6px_rgba(0,120,212,0.8)]' : mlLoading ? 'bg-amber-400 animate-pulse' : 'bg-red-500 shadow-[0_0_6px_rgba(239,68,68,0.8)]'}`} />
      ...
      <div
        className="flex flex-col h-full flex-shrink-0 transition-all duration-300 ease-out overflow-hidden z-10"
        style={{
          width: chatExpanded ? '320px' : '0px',
          marginLeft: chatExpanded ? '16px' : '0px'
        }}
      >
        <div className="w-[320px] h-full flex flex-col flex-shrink-0 glass-panel rounded-3xl overflow-hidden">
          {/* Chat Header */}
          <div className="px-5 py-4 border-b border-white/[0.06] bg-black/25 flex-shrink-0 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500/20 to-purple-500/20 border border-blue-500/35 flex items-center justify-center shadow-inner">
                  <span className="text-base select-none">💬</span>
                </div>
                <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-blue-500 border-2 border-[#080808] shadow-[0_0_8px_rgba(0,120,212,0.5)]" />
              </div>
              <div>
                <h3 className="text-sm font-semibold text-white/90 tracking-wide">Receiver Node</h3>
                <p className="text-[11px] font-medium text-blue-400">Connected</p>
              </div>
            </div>
            <div className="flex items-center gap-1.5">
              {messages.length > 0 && (
